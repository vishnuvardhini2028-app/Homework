...
select s_name,dept_name from studentts inner join departments on studentts.dept_id=departments.dept_id where dept_name='EEE'
select dept_name,count(s_id) as total_studentts from studentts inner join departments on studentts.dept_id=departments.dept_id group by dept_name
select s_name,dept_name from studentts inner join departments on studentts.dept_id=departments.dept_id order by s_name,dept_name
select s_name,dept_name from studentts inner join departments on studentts.dept_id=departments.dept_id where dept_name='IT' and s_id>2
